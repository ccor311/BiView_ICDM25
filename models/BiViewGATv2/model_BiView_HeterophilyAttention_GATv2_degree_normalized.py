...
from models.BiViewGATv2.DegreeNormalizedHeterophilyAttentionGATv2 import DegreeNormalizedEdgeGATv2MaskConv
from torch_geometric.nn import global_mean_pool as gap, global_max_pool as gmp
import torch.nn.functional as F
from torch_geometric import utils
from torch_sparse import mul

class BiViewDegreeNormalized(torch.nn.Module):
    def __init__(self, args):
        super(BiViewDegreeNormalized, self).__init__()
        self.args = args
        ...
        for i in range(self.num_layers):
            ...
            self.hom_conv_layers.append(GATv2Conv(in_features, out_features, edge_dim=1))

            #Heterophily Conv Layer
            self.het_conv_layers.append(DegreeNormalizedEdgeGATv2MaskConv(in_features, out_features, edge_dim=1))
    ...
    def forward(self, data, last_epoch):
        x, edge_index, batch, hom_mask, het_mask, hom_edge_degrees, het_edge_degrees = data.x, data.edge_index, data.batch, data.homophily_mask, data.heterophily_mask, data.hom_edge_degrees, data.het_edge_degrees
        ...
        x_het = F.relu(self.het_conv_layers[0](x, edge_index, het_mask, edge_degrees=het_edge_degrees))
        #Concat
        ...
        for i in range(1,self.args.num_layers):
            ...
            x_het = F.relu(het_conv_layer(x, edge_index, het_mask, edge_degrees=het_edge_degrees))
            x = torch.cat((x_hom, x_het), dim=1)
            readout_list.append(torch.cat([gmp(x, batch), gap(x, batch)], dim=1))
