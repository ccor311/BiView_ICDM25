...
class HetGATv2Conv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #Edge Attention
        self.edge_att = Parameter(torch.Tensor(1, int(heads / 2), out_channels))
        ...
        if self.add_self_loops:
            if isinstance(edge_index, Tensor):
                ...
                if masks is not None:
                    loop_mask = torch.ones((num_nodes,), dtype=torch.bool, device=masks[0].device)
                    for i in range(len(masks)):
                        masks[i] = torch.cat([masks[i], loop_mask], dim=0)
    ...
    def message(self, x_j: Tensor, x_i: Tensor, edge_attr: OptTensor,
                ...
                size_i: Optional[int],
                masks: list[OptTensor]) -> Tensor:
        x = x_i + x_j
        #Get distance between node representations
        edge_distance = (x_i[:, 1, :] - x_j[:, 1, :]) ** 2
        ...
        for i in range(self.heads):
            if i % 2 == 0:
                alpha[:, i] = alpha[:, i] * masks[0][:, 0]
            else:    
                alpha[:, i] = alpha[:, i] * masks[1][:, 0] 
        self._alpha = alpha
