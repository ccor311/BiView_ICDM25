...
class BernConv(MessagePassing):
    ...
    def forward(self,x,edge_index,coe,edge_weight=None, mask=None):
        ...
        if mask is not None:
            ...
            mask2 = torch.cat([mask, loop_mask], dim=0)
        tmp=[]
        tmp.append(x)
        if mask is not None:
            for i in range(self.K):
                    x=self.propagate(edge_index2,x=x,norm=norm2,size=None, mask=mask2)
                    tmp.append(x)
            ...
            for i in range(self.K):
                    x=tmp[self.K-i-1]
                    x=self.propagate(edge_index1,x=x,norm=norm1,size=None, mask=mask)
                    for j in range(i):
                            x=self.propagate(edge_index1,x=x,norm=norm1,size=None, mask=mask)
    ...
    def message(self, x_j, norm, mask=None):
        if mask is not None:
            mask = mask[:, None]
            x_j = x_j * mask
